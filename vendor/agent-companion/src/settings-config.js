...
/** @returns {Settings} */
export const defaultSettings=()=>({version:1,sources:/** @type {Record<SourceId, SourceConfig>} */(Object.fromEntries(SOURCE_IDS.map(id=>[id,{enabled:true,path:''}]))),monitor:{avatarStyle:'animal',railVisibleCount:8,autoDiscover:true,retentionHours:.5,assignment:'auto',seats:Array(8).fill('auto')},scene:{light:'day',weather:'clear',lightning:true,door:false,ceiling:false,playing:true,speed:1,maxFps:60,renderResolution:'native',showPerformance:false,reducedMotion:false,defaultView:'program'},notifications:{desktop:false,wait:true,error:true,done:true,sound:false},general:{mode:'live',rememberView:true},schedule:{enabled:false,start:'09:00',end:'18:00',deferBusy:true}});
/**
...
export function validateSettings(input){
 ...
 for(const id of SOURCE_IDS){const s=raw.sources?.[id];if(!s||typeof s.enabled!=='boolean'||typeof s.path!=='string'||s.path.length>2048||s.path.includes('\0'))throw Error('Agent 配置无效');d.sources[id]={enabled:s.enabled,path:s.path.trim()};}
